...
class WhisperEngine {
  ...
  getModelPath() {
    const dir = this.getWhisperDir();
    const candidates = ["ggml-tiny.bin", "ggml-base.bin", "ggml-small.bin"];
    for (const name of candidates) {
      const candidate = path.join(dir, name);
      if (fs.existsSync(candidate)) return candidate;
    }
    return path.join(dir, "ggml-tiny.bin");
  }
  ...
  async transcribePayload(payload) {
    if (!this.isReady()) {
      throw new Error("Whisper no está instalado en este OnniVers. Ejecuta npm run desktop:whisper-build.");
    }
