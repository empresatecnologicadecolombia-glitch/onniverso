...
function ColiseoSceneContent({
  ...
  allowScreenInteraction: boolean;
}) {
  ...
  return (
    <>
      ...
      <ColiseoFloatingWebViewScreen
        ...
        allowInteraction={allowScreenInteraction}
      />
      <ColiseoFloatingPdfScreen
        onScreenPointerDown={onScreenPointerDown}
        allowInteraction={allowScreenInteraction}
      />
...
export default function ColiseoImmersiveScene({
  ...
  const allowScreenInteraction = !studentViewLocked;
  ...
  const handleScreenPointerDown = useCallback(() => {
    if (studentViewLocked) return;
    setScreenInteracting(true);
    if (document.pointerLockElement) document.exitPointerLock();
  }, [studentViewLocked]);

  const pointerLockEnabled =
    usesPointerLock && !useNativeWebView && !screenInteracting && !studentViewLocked;
  ...
  return (
    <div className={`relative h-[100dvh] w-full [&_*]:outline-none ${mixedRealityActive ? "bg-transparent" : "bg-black"}`}>
      <Canvas
        className={`relative z-10 ${studentViewLocked ? "pointer-events-none touch-none" : "touch-none"}`}
      ...
      >
        <Suspense fallback={null}>
          <ColiseoSceneContent
            ...
            allowScreenInteraction={allowScreenInteraction}
          />
        </Suspense>
      ...
      <p className="pointer-events-none absolute bottom-4 left-1/2 z-10 max-w-md -translate-x-1/2 px-4 text-center text-[11px] text-slate-400">
        {studentViewLocked
          ? "Vista y pantalla sincronizadas por el docente"
          : useNativeWebView
